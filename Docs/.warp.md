...
├── awards.html, scholarship.html    # Application pages
├── tickets.html                    # Tickets (Membee embed)
├── newsletter.php                  # Newsletter backend
├── assets/                         # Images, video, app.js, app.css
├── Docs/                           # Project documentation (this file included)
├── .gitignore                      # Git ignore rules (ignores _backup/)
└── _backup/                        # Local archives (ignored by Git)
```
...
- Navigation (desktop/mobile): Event, Tickets, Applications, Newsletter, EN | FR
- Section order: Hero → Event Details → Applications → 2024 Recap → Newsletter → Footer
...
### Image Management
- Logos in `assets/` (e.g., `assets/able_logo.png`, partner logos)
...
8. Place all images/videos in `assets/`; keep logic in `assets/app.js`
...
- Verify assets/able_logo.png exists
...
- Keep functions modular in `assets/app.js` (setLanguage, initVideoPlayer, initNewsletter, initNewsletterUX, initScrollSpy)
...
1. **Header**: Logo, simplified navigation (Event, Tickets, Applications, Newsletter), EN/FR toggle (desktop) + mobile hamburger menu
2. **Hero**: Three logos (Toronto Police, ABLE, TPA) with subtle rings; centered date block; primary CTAs
3. **Event Details**: Bilingual event information in two columns incl. pricing/time/venue
4. **Applications**: Awards and Scholarship applications (CTA and links)
5. **2024 Video Recap**: Custom video player with bilingual content and statistics
6. **Tickets**: Dedicated tickets page with Membee embed
7. **Newsletter**: Email signup with PHP backend
...
### Deployment Status
- Meta titles updated to “A.B.L.E 33rd Annual Scholarship and Awards Gala”
